...
def simuler2(n,k):
    ...
    while continuer == True:
        plt.pause(0.01)
        plt.clf()
        afficher(propagation_knn(mouvement(M),1,0.5,0.3))
    return()
...
def propagation_knn(M,k,pr,pm):
    ...
    for i in range(len(M)):
        for j in range(len(M)):
            if M[i][j]==2 and bernoulli(pm)==1:
                M2[i][j]=4
            elif M[i][j]==2 and bernoulli(pr)==1:
                M2[i][j]=3
            else:
                if M[i][j]==1 and knn(M,(i,j),k)==2:
                    M2[i][j]=2
    return M2
...
def simulation(n,v,pr,pm):
    ...
    C=compte(M)
    t=0
    S,I,R,Mo,T,popl = [C[1]],[C[2]],[C[3]],[C[4]],[0],[n]
    continuer=True
    while continuer==True:
        t+=1
